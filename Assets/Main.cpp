...
vector<char_node_t*> getDFS(char_node_t* root, char target)
{
	...
	while (!node_stack.empty())
	{
		...
		for (char_node_t* child : current->children) // <get child in children>
		{
			...
			if (!child->visited) // <child not visited>
			{
				...
				node_stack.push(child); // <push child onto node_stack>
				popNode = false;
				break;
			}
		}
	...
	while (!node_stack.empty())
	{
		...
		path.insert(path.begin(), node_stack.top()); // <begin() of path>, <top of node_stack>
		node_stack.pop(); // <pop node_stack>
	}
...
vector<char_node_t*> getBFS(char_node_t* root, char target)
{
	...
	while (!node_queue.empty())
	{
		...
		char_node_t* current = node_queue.front(); // <get front of queue>
		...
		node_queue.pop(); // <pop the front of the node_queue>
	}
	...
	while (node)
	{
		...
		path.insert(path.begin(), node); // <begin() of path>
...
int main()
{
	...
	// display path result
	cout << "path: ";
	for (char_node_t* node : path)// <iterate through all the path nodes>
	{
